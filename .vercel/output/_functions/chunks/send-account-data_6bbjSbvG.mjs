...
//#region src/pages/api/send-account-data.ts
var send_account_data_exports = /* @__PURE__ */ __exportAll({
	POST: () => POST,
	prerender: () => false
});
...
var POST = async ({ request }) => {
	try {
		...
		const { to, userData, userGroups, allExpenses, notifications } = await request.json();
		if (!to) return new Response(JSON.stringify({ error: "Missing recipient email" }), { status: 400 });
		...
		const groupsHtml = (userGroups || []).map((g) => `
...
//#endregion
//#region \0virtual:astro:page:src/pages/api/send-account-data@_@ts
var page = () => send_account_data_exports;
//#endregion
export { page };
